...
| v0.3.0 测试 APK | `test.001` 自动构建通过但用户实机发现词条主WebView被403拦截，候选不通过；功能2 v0.1.1定向修复中，待生成 `test.002` |
| 跨对话治理 | 独立 worktree 和固定格式汇报已跑通；首次全局 memory 误读已审计、修订规则并二次确认通过 |
| 设备实机验证 | 用户截图已确认App可进入词典并查询，但 `test.001` 词条HTML加载失败；其他旧数据、长按/返回、管理/导入/发音仍未确认 |
