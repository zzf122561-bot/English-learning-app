...
| 设备覆盖安装 | 待用户自测 | 必须覆盖现有版本，不卸载、不清数据 |
| 锚点/发音/字号/Room迁移 | 待用户自测 | 自动测试不能证明WebView、TTS、UI或真实迁移设备行为 |
| 正式发布 | 未完成 | 用户确认同一APK后才能提升并创建`app-v1.0.0`标签 |

## 设备边界与音源说明

- 总控没有连接或操作Android设备，不能把自动构建写成实机通过。
- 两本随包内置词典的MDD中没有检测到音频资源，且抽样词条没有显式HTTPS音频引用；正常预期是回退Android英语TTS，不得宣称为MDD原声。
- `F02-LINK-001`、`F02-AUDIO-001`目前为implemented / awaiting device verification，用户复测前不得标记resolved。
