...
| 设备覆盖安装 | 待用户自测 | 用户自行直接覆盖当前已安装版本；不得卸载或清除数据 |
| 用户功能确认 | 待执行 | 需按同目录 `ACCEPTANCE.md` 核对旧数据、双入口、查词、管理、导入、图片与发音 |

本产物在用户确认前不是正式版本，不创建 `app-v0.3.0` 标签。用户确认后提升同一 APK，不重新生成不同二进制。
