...
| 设备覆盖安装 | 待用户自测 | 必须直接覆盖现有 App，不卸载、不清除数据 |
| WebView缺陷复测 | 待用户自测 | 必须确认 `another place` 正文、CSS和图片不再出现原错误 |
| 用户完整功能确认 | 待完成 | 旧数据、功能1、双入口、快速查词、词典管理/导入/发音仍需真实反馈 |

## 历史边界

- test.001 的实机失败结论和 APK 永久保留，不因本次自动构建通过而改写。
- test.002 尚未获得设备运行证据，不创建 `app-v0.3.0` 标签，也不标记正式发布。
